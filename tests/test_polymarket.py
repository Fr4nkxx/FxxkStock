...
import fxxkstock.dataflows.config as config_module
import fxxkstock.default_config as default_config
from fxxkstock.dataflows import interface, polymarket
from fxxkstock.dataflows.config import set_config


def _market(question, prob, *, volume, end_date, closed=False, wk=None, slug=None):
    return {
        "question": question,
        "slug": slug,
        "outcomes": '["Yes", "No"]',
...
_SEARCH = {
    "events": [
        {
            "markets": [
                _market("Open big?", 0.76, volume=5_000_000, end_date="2030-12-31T00:00:00Z", wk=-0.045, slug="open-big"),
...
class PolymarketFormatTests(unittest.TestCase):
    def test_probability_volume_and_weekly_change_render(self):
        ...
        self.assertIn("Link: https://polymarket.com/event/open-big", out)
