...
class TestTemperatureEnvOverlay:
    def test_env_sets_temperature(self, monkeypatch):
        import fxxkstock.default_config as dc
        monkeypatch.setenv("FXXKSTOCK_TEMPERATURE", "0.2")
        importlib.reload(dc)
        ...
        monkeypatch.delenv("FXXKSTOCK_TEMPERATURE", raising=False)
        importlib.reload(dc)
...
class TestProviderKwargsTemperature:
    ...
    def _kwargs_for(self, temperature):
        ...
        return FxxKStockGraph._get_provider_kwargs(graph)
