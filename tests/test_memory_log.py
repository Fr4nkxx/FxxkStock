...
from fxxkstock.agents.managers.portfolio_manager import create_portfolio_manager
...
class TestDeferredReflection:
    ...
    def test_fetch_returns_valid_ticker(self):
        ...
        with patch("yfinance.Ticker") as mock_ticker_cls:
            ...
            raw, alpha, days = FxxKStockGraph._fetch_returns(mock_graph, "NVDA", "2026-01-05")
    ...
    def test_fetch_returns_too_recent(self):
        ...
        with patch("yfinance.Ticker") as mock_ticker_cls:
            ...
            raw, alpha, days = FxxKStockGraph._fetch_returns(mock_graph, "NVDA", "2026-04-19")
    ...
    def test_fetch_returns_delisted(self):
        ...
        with patch("yfinance.Ticker") as mock_ticker_cls:
            ...
            mock_ticker_cls.return_value = m
            raw, alpha, days = FxxKStockGraph._fetch_returns(mock_graph, "XXXXXFAKE", "2026-01-10")
        assert raw is None and alpha is None and days is None
    ...
    # FxxKStockGraph._resolve_benchmark — picks index for alpha calc
    ...
    def test_resolve_benchmark_explicit_override(self):
        ...
        assert FxxKStockGraph._resolve_benchmark(mock_graph, "7203.T") == "QQQ"
        assert FxxKStockGraph._resolve_benchmark(mock_graph, "NVDA") == "QQQ"
    ...
    def test_resolve_benchmark_suffix_map(self):
        ...
        assert FxxKStockGraph._resolve_benchmark(mock_graph, "7203.T") == "^N225"
        assert FxxKStockGraph._resolve_benchmark(mock_graph, "0700.HK") == "^HSI"
        assert FxxKStockGraph._resolve_benchmark(mock_graph, "RELIANCE.NS") == "^NSEI"
        assert FxxKStockGraph._resolve_benchmark(mock_graph, "AZN.L") == "^FTSE"
    ...
    def test_resolve_benchmark_china_a_shares(self):
        ...
        assert FxxKStockGraph._resolve_benchmark(mock_graph, "600519.SS") == "000001.SS"
        assert FxxKStockGraph._resolve_benchmark(mock_graph, "000001.SZ") == "399001.SZ"
    ...
    def test_resolve_benchmark_us_ticker_defaults_to_spy(self):
        ...
        assert FxxKStockGraph._resolve_benchmark(mock_graph, "NVDA") == "SPY"
        assert FxxKStockGraph._resolve_benchmark(mock_graph, "AAPL") == "SPY"
    ...
    def test_resolve_benchmark_unknown_suffix_falls_back(self):
        ...
        assert FxxKStockGraph._resolve_benchmark(mock_graph, "FAKE.XX") == "SPY"
        assert FxxKStockGraph._resolve_benchmark(mock_graph, "BRK.B") == "SPY"
    ...
    # FxxKStockGraph._resolve_pending_entries
    ...
    def test_resolve_skips_other_tickers(self, tmp_path):
        ...
        FxxKStockGraph._resolve_pending_entries(mock_graph, "NVDA")
...
class TestPortfolioManagerInjection:
    ...
    def test_pm_prompt_includes_position_context(self):
        ...
        state["position_context"] = {"status": "flat"}

        pm_node(state)

        assert "Current account position: FLAT" in captured["prompt"]
        assert "never anchor the decision on breaking even" in captured["prompt"]
        assert "market low, technical level, prior-report price" in captured["prompt"]
...
class TestLegacyRemoval:
    ...
    def test_financial_situation_memory_removed(self):
        ...
        import fxxkstock.agents.utils.memory as m
    ...
    def test_reflect_and_remember_removed(self):
        """FxxKStockGraph must not expose reflect_and_remember."""
        assert not hasattr(FxxKStockGraph, "reflect_and_remember")
    ...
    def test_full_pipeline_no_regression(self, tmp_path):
        ...
        mock_graph.ticker_memory = TickerMemoryStore({
            "ticker_memory_dir": str(tmp_path / "tickers"),
            "reports_dir": str(tmp_path / "reports"),
        })
        mock_graph.selected_analysts = ("market", "social", "news", "fundamentals")
        ...
        mock_graph._run_graph = functools.partial(
            FxxKStockGraph._run_graph, mock_graph
        )
        mock_graph.prepare_run = functools.partial(
            FxxKStockGraph.prepare_run, mock_graph
        )
        mock_graph.finalize_run = functools.partial(
            FxxKStockGraph.finalize_run, mock_graph
        )
        FxxKStockGraph.propagate(mock_graph, "NVDA", "2026-01-10")
