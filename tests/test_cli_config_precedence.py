...
def test_research_depth_sets_both_rounds_without_env(monkeypatch):
    for var in ("FXXKSTOCK_MAX_DEBATE_ROUNDS", "FXXKSTOCK_MAX_RISK_ROUNDS"):
...
def test_env_round_counts_win_over_selection(monkeypatch):
    monkeypatch.setenv("FXXKSTOCK_MAX_DEBATE_ROUNDS", "2")
    monkeypatch.setenv("FXXKSTOCK_MAX_RISK_ROUNDS", "4")
...
def test_partial_env_only_overrides_that_count(monkeypatch):
    monkeypatch.setenv("FXXKSTOCK_MAX_DEBATE_ROUNDS", "2")
    monkeypatch.delenv("FXXKSTOCK_MAX_RISK_ROUNDS", raising=False)
