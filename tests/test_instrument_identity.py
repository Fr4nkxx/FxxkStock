...
class ResolveInstrumentIdentityTests(unittest.TestCase):
    ...
    def test_resolves_company_metadata_from_yfinance(self):
        with patch("fxxkstock.agents.utils.agent_utils.yf.Ticker") as mock:
        ...
        self.assertEqual(identity["currency"], "JPY")
    ...
    def test_fails_open_on_exception(self):
        with patch(
            "fxxkstock.agents.utils.agent_utils.yf.Ticker",
...
class BuildInstrumentContextTests(unittest.TestCase):
    ...
    def test_injects_fx_instruction_for_usd(self):
        context = build_instrument_context(
            "NVDA",
            "stock",
            {
        ...
        self.assertIn("Source data quote currency: USD", context)
        self.assertIn("1 USD = 7.2500 CNY", context)

    def test_cny_skips_fx_instruction(self):
        context = build_instrument_context(
            "600519.SS",
            "stock",
            {"company_name": "贵州茅台", "currency": "CNY"},
        )
        self.assertNotIn("Source data quote currency", context)

    def test_forbids_translating_company_name(self):
        context = build_instrument_context(
            "603629.SS",
            "stock",
            {"company_name": "利通电子", "currency": "CNY"},
        )
        self.assertIn("Company: 利通电子", context)
        self.assertIn("never translate, romanize, or rewrite it", context)
