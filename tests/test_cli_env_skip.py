...
The config-layer override (FXXKSTOCK_* -> DEFAULT_CONFIG) is covered by
...
class TestCliSkipsPromptsFromEnv(unittest.TestCase):
    def test_env_config_skips_llm_prompts(self):
        ...
        env = {
            "FXXKSTOCK_LLM_PROVIDER": "openai",
            "FXXKSTOCK_DEEP_THINK_LLM": "kimi-k2.5",
            "FXXKSTOCK_QUICK_THINK_LLM": "deepseek-v4-pro",
            "FXXKSTOCK_LLM_BACKEND_URL": "https://opencode.ai/zen/go/v1",
            "FXXKSTOCK_OUTPUT_LANGUAGE": "Japanese",
        }
...
class TestResearchDepthSkippedFromEnv(unittest.TestCase):
    def test_both_round_envs_skip_depth_prompt(self):
        ...
        env = {
            "FXXKSTOCK_MAX_DEBATE_ROUNDS": "2",
            "FXXKSTOCK_MAX_RISK_ROUNDS": "4",
        }
        fake_cfg = dict(m.DEFAULT_CONFIG)
...
class TestReasoningEffortSkippedFromEnv(unittest.TestCase):
    def test_effort_env_skips_step8_prompt(self):
        ...
        env = {"FXXKSTOCK_OPENAI_REASONING_EFFORT": "high"}
        fake_cfg = dict(m.DEFAULT_CONFIG)
