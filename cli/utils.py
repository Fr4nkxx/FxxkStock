...
from fxxkstock.llm_clients.api_key_env import get_api_key_env
from fxxkstock.llm_clients.model_catalog import get_model_options
...
def ask_position_context() -> dict:
    """Collect an optional, per-run position without persisting it."""
    status = questionary.select(
        "Current position for this ticker:",
        choices=[
            questionary.Choice("Skip (not provided)", "unknown"),
            questionary.Choice("Flat (no position)", "flat"),
            questionary.Choice("Held", "held"),
        ],
    ...
    def positive_number(value: str):
        try:
            return float(value.strip()) > 0 or "Enter a number greater than zero."
        except ValueError:
...
def resolve_backend_url(
    ...
    An explicit env override (``env_url``, from ``FXXKSTOCK_LLM_BACKEND_URL``
...
def ensure_api_key(provider: str) -> str | None:
    ...
    from fxxkstock.llm_clients.openai_client import OPENAI_COMPATIBLE_PROVIDERS
...
def ask_chrome_platform(default: str = "ubuntu") -> str:
    """Select the desktop platform used to launch Google Chrome."""
    choices = [
        questionary.Choice("macOS", "macos"),
        questionary.Choice("Windows", "windows"),
        questionary.Choice("Ubuntu", "ubuntu"),
    ]
    selected = questionary.select(
        "Select the Google Chrome platform:",
        choices=choices,
        default=default,
        style=questionary.Style([("selected", "fg:green noinherit")]),
    ).ask()
