...
app = typer.Typer(
    name="FxxKStock",
    help="FxxKStock CLI: Multi-Agents LLM Financial Trading Framework",
...
def update_display(layout, spinner_text=None, stats_handler=None, start_time=None):
    ...
    layout["header"].update(
        Panel(
            "[bold green]Welcome to FxxKStock CLI[/bold green]\n"
            "[dim]Multi-agent stock research workspace[/dim]",
            title="Welcome to FxxKStock",
            border_style="green",
            padding=(1, 2),
            expand=True,
        )
...
def get_user_selections():
    ...
    welcome_content += "[bold green]FxxKStock: Multi-Agents LLM Financial Trading Framework - CLI[/bold green]\n\n"
    welcome_content += "[bold]Workflow Steps:[/bold]\n"
    welcome_content += "I. Analyst Team → II. Research Team → III. Trader → IV. Risk Management → V. Portfolio Management\n\n"
    welcome_content += "[dim]FxxKStock local research workspace[/dim]"
    ...
    if os.environ.get("FXXKSTOCK_CHROME_PLATFORM"):
        chrome_platform = DEFAULT_CONFIG["cn_browser_platform"]
        console.print(
            f"[green]✓ Chrome platform from environment:[/green] {chrome_platform}"
        )
    else:
        console.print(
            create_question_box(
                "Chrome Platform",
                "Select the operating system used to launch Google Chrome with CDP",
                DEFAULT_CONFIG["cn_browser_platform"],
            )
        )
        chrome_platform = ask_chrome_platform(DEFAULT_CONFIG["cn_browser_platform"])

    # Step 3: Output language (skipped when set via FXXKSTOCK_OUTPUT_LANGUAGE)
    if os.environ.get("FXXKSTOCK_OUTPUT_LANGUAGE"):
    ...
    # supplied through FXXKSTOCK_MAX_DEBATE_ROUNDS / _MAX_RISK_ROUNDS we keep
    # the run non-interactive and honor the env values (#977).
    depth_from_env = bool(os.environ.get("FXXKSTOCK_MAX_DEBATE_ROUNDS")) and bool(
        os.environ.get("FXXKSTOCK_MAX_RISK_ROUNDS")
    )
    if depth_from_env:
    ...
    # Step 6: LLM Provider (skipped when set via FXXKSTOCK_LLM_PROVIDER).
    # The backend URL comes from FXXKSTOCK_LLM_BACKEND_URL when set,
    # otherwise the provider's default endpoint — the same value the menu
    # would have picked.
    provider_from_env = bool(os.environ.get("FXXKSTOCK_LLM_PROVIDER"))
    ...
    if os.environ.get("FXXKSTOCK_QUICK_THINK_LLM") or os.environ.get("FXXKSTOCK_DEEP_THINK_LLM"):
    ...
    # settable via its FXXKSTOCK_* env var; when that var is set (or the
...
def _build_run_config(selections: dict, checkpoint: bool | None) -> dict:
    ...
    # (FXXKSTOCK_MAX_DEBATE_ROUNDS / _MAX_RISK_ROUNDS) wins over the
    # interactive selection — leave the env-applied value in place (#977).
    if not os.environ.get("FXXKSTOCK_MAX_DEBATE_ROUNDS"):
        config["max_debate_rounds"] = selections["research_depth"]
    if not os.environ.get("FXXKSTOCK_MAX_RISK_ROUNDS"):
    ...
    config["cn_browser_platform"] = selections.get(
        "chrome_platform", DEFAULT_CONFIG["cn_browser_platform"]
    )
    # --checkpoint/--no-checkpoint overrides only when explicitly given; omitting
    # the flag preserves FXXKSTOCK_CHECKPOINT_ENABLED / the default (#976).
...
def run_analysis(checkpoint: bool | None = None):
    ...
    with Live(layout, refresh_per_second=4):
        ...
        graph.inject_market_region(selections["ticker"])
        ...
        init_agent_state = graph.propagator.create_initial_state(
            ...
            position=selections.get("position"),
        )
...
def analyze(
    checkpoint: bool | None = typer.Option(
        ...
        "crashed run can resume). Omit to honor FXXKSTOCK_CHECKPOINT_ENABLED.",
    ),
    ...
    if clear_checkpoints:
        from fxxkstock.graph.checkpointer import clear_all_checkpoints
