"""Append-only markdown decision log for FxxKStock."""

import re
from pathlib import Path

from fxxkstock.agents.utils.rating import parse_rating
...
class TradingMemoryLog:
    ...
    def __init__(self, config: dict = None):
        ...
        if path:
            ...
            legacy = cfg.get("memory_log_legacy_path")
            if legacy and not self._log_path.exists():
                legacy_path = Path(legacy).expanduser()
                if legacy_path.is_file() and legacy_path.resolve() != self._log_path.resolve():
