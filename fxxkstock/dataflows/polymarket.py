...
def _polymarket_market_url(market: dict, event: dict | None = None) -> str:
    """Build a public Polymarket page URL when slug is available."""
    slug = market.get("slug") or (event or {}).get("slug")
    if slug:
...
def get_prediction_markets(topic: str, limit: int | None = None) -> str:
    ...
    candidates.sort(key=lambda pair: pair[0].get("volumeNum") or 0, reverse=True)
    ...
    lines = []
    for m, event in candidates[:limit]:
        ...
        url = _polymarket_market_url(m, event)
        if url:
            line += f"\n  Link: {url}"
        lines.append(line)
