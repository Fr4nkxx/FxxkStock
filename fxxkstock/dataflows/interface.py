...
VENDOR_METHODS = {
    ...
    # news_data
    "get_news": {
        "browser": get_browser_news,
        "eastmoney": get_eastmoney_news,
        "alpha_vantage": get_alpha_vantage_news,
        "yfinance": get_news_yfinance,
    },
    "get_global_news": {
        "browser": get_browser_global_news,
        "eastmoney": get_eastmoney_global_news,
        "yfinance": get_global_news_yfinance,
        "alpha_vantage": get_alpha_vantage_global_news,
    },
    "get_insider_transactions": {
        "browser": get_browser_insider,
        "cninfo": get_cninfo_insider,
...
def _build_vendor_chain(method: str, explicit: list[str], all_available: list[str]) -> list[str]:
    """Build ordered vendor chain, prepending CN vendors when market_region is CN."""
    config = get_config()
...
def route_to_vendor(method: str, *args, **kwargs):
    ...
    if explicit:
        vendor_chain = _build_vendor_chain(method, explicit, all_available_vendors)
        if not vendor_chain:
            raise ValueError(
        ...
        vendor_chain = _build_vendor_chain(method, [], all_available_vendors)
    ...
    for vendor in vendor_chain:
        ...
        try:
            ...
            logger.warning(
                "Vendor %r browser/CDP unavailable for %s; trying next vendor: %s",
                vendor,
                method,
                e,
            )
