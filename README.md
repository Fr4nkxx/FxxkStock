...
- 最终 Markdown 报告查看。
- 节点日历，按月份查看复盘、执行和风险工作，并集中列出事件与条件节点。

### 实际界面

以下截图来自本地实际运行界面和已有分析报告数据。

#### Web 研究工作台

![FxxKStock Web 研究工作台](./docs/images/web-workbench.png)

#### 节点日历

![FxxKStock 节点日历](./docs/images/node-calendar.png)

### 节点日历

主页顶部的“节点日历”按钮会打开 `/calendar`。节点来自每只标的最新一份分析
报告，而不是由前端临时生成。Portfolio Manager 在最终决策中输出结构化
`Review Nodes`，报告保存时写入：

```text
reports/<TICKER>/<YYYYMMDD_HHMMSS>/calendar_nodes.json
```

节点分为：

- **日期节点**：拥有明确的 ISO 日期，例如复盘日、执行截止日和风险检查日，显示在月历及未来 7 天视图。
- **事件节点**：事件尚无可靠日期，例如“下一融资余额报告发布日”，显示在条件触发视图。
- **执行与风险条件**：最终决策中的 `Execution Condition` 和 `Risk Boundary` 会进入条件视图；其中包含的明确日期还会额外生成日历节点。

为避免模型写错星期，模型只负责输出 `YYYY-MM-DD` 日期，不输出星期。日期由
Pydantic 校验，日历页面再根据日期计算“周一/周二”等显示。模型在执行条件、
风险边界或复盘节点中提到的每个明确日期，都必须同步提供对应的结构化日期节点；
后端还会做一次确定性补充解析，防止日期只留在正文中。

旧报告没有 `calendar_nodes.json` 时，Web 端会从固定的 `Review Trigger`、
`Execution Condition` 和 `Risk Boundary` 字段回填明确日期及事件，因此无需为了
显示历史节点重新运行分析。日历 API 为：

```text
GET /api/calendar/nodes
```

当前版本负责节点的生成、保存、聚合与可视化；条件节点尚不包含后台定时行情监控、
系统通知或自动交易功能，需要用户在日历中主动检查。
...
```text
reports/<TICKER>/<YYYYMMDD_HHMMSS>/
```
...
| 目录 | 内容 |
|---|---|
| `reports/` | 按 `代码/时间/` 保存完整 Markdown 报告、节点 JSON 与校准快照 |
